const addresses = require("../addresses/judas") // Get all relevant Ethereum and BSC addresses
const judasAbi = require("../abi/judas.json") // Get the token ABI for the project. ABIs can be found on the Etherscan page for the contract if the contract has been verified. Otherwise you may need to ask your Solidity dev for it.
...
const getProjectOneData = async (web3s) => {
    ...
    let judas = new web3.eth.Contract(judasAbi, eth_addresses.contract)
    // Here we instantiate the BSC smart contract object
    let bsc_judas
    try {
        bsc_judas = new bsc_web3.eth.Contract(judasAbi, bsc_addresses.contract)
    }
    catch(err) {
    ...
    const burnt_on_eth = await judas.methods.balanceOf(eth_addresses.burnt).call() 

    tokenData.eth.totalSupply.value = await judas.methods.totalSupply().call()
    const team_1 = await judas.methods.balanceOf(eth_addresses.team_1).call() 
    const team_2 = await judas.methods.balanceOf(eth_addresses.team_2).call() 
    const team_3 = await judas.methods.balanceOf(eth_addresses.team_3).call() 
  
     // Get base BSC values 
    try {
        burnt_on_bsc = await bsc_judas.methods.balanceOf(bsc_addresses.burnt).call() 
    }
    catch(err){
    ...
    try {
        tokenData.bsc.totalSupply.value = await bsc_judas.methods.totalSupply().call()
    }
    catch(err){
    ...
    let  bsc_team_1 
    try {
         bsc_team_1 = await bsc_judas.methods.balanceOf(bsc_addresses.team_1).call() 
    }
    catch(err){
    ...
    let bsc_team_2 
    try {
        bsc_team_2 = await bsc_judas.methods.balanceOf(bsc_addresses.team_2).call() 
    }
    catch(err){
    ...
    tokenData.bsc.totalSupply.description = "Total supply of judas on BSC"
  
    tokenData.eth.circulatingSupply.description = "Circulating supply of judas on ETH"
    tokenData.bsc.circulatingSupply.description = "Circulating supply of judas on BSC"
  
    tokenData.combined.totalSupply.description = "Total supply of judas (BSC & ETH)"
    tokenData.combined.circulatingSupply.description = "Circulating supply of judas (BSC & ETH)"
  
    // Set names
  
    tokenData.eth.totalSupply.name = "Total Supply of judas on ETH"
    tokenData.bsc.totalSupply.name = "Total Supply of judas on BSC"
  
    tokenData.eth.circulatingSupply.name = "Circulating Supply of judas on ETH"
    tokenData.bsc.circulatingSupply.name = "Circulating Supply of judas on BSC"
  
    tokenData.combined.totalSupply.name = "Total Supply of judas on (BSC & ETH)"
    tokenData.combined.circulatingSupply.name = "Circulating Supply of judas on (BSC & ETH)"
    ...
    try {
      ...
      db.updatejudasData(tokenData, client) 
    }
    catch(err) {
      console.log(err)
    }
